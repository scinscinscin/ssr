...
export function stringifyAstNode(ast: ASTNode) {
  const stringified: string = match(ast)
    .with({ type: "object" }, (res: any) => res.name)
    .with({ type: "array" }, (res: any) => `[${stringifyAstNode(res.insideType)}]`)
    .with({ type: "fieldResolver" }, (res: any) => `${stringifyAstNode(res.returnType)}`)
    .with({ type: "string" }, () => "String")
    .with({ type: "number" }, () => "Int")
